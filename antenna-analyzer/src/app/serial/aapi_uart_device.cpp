...
class AAPiUartPrivate : public AAPiObject
{
    friend class AAPiUartDevice;

    typedef std::list<AAPiUartPrivate*> TCache;
    typedef std::recursive_mutex TMutex;

    DECLARE_AAPI_OBJECT(AAPiUartPrivate)
...
public:
    ...
    static void set(AAPiUartPrivate *ctx);
    static bool remove(AAPiUartPrivate *ctx);
...
AAPiUartPrivate::TCache AAPiUartPrivate::m_cache;
AAPiUartPrivate::TMutex AAPiUartPrivate::m_mtx;
...
int AAPiUartDevice::enumerate(AAPiArray<AAPiString>& device_names)
{
    ...
    AAPiArray<AAPiString> ports1, ports2, ports3;
    struct dirent *entry;
    int fd;
    ...
    {
        device_names.append(ports1[i]);
    }
    for (int i=0; i < ports2.size(); i++)
    {
        device_names.append(ports2[i]);
    }
    for (int i=0; i < ports3.size(); i++)
    {
        device_names.append(ports3[i]);
    }
    return AAPI_SUCCESS;
}

int AAPiUartDevice::open(const char *device_name, struct AAPiUartParams* uart_params)
{
    int                     fd;
    ...
    saio.sa_handler = AAPiUartPrivate::io_sig_handler;
    ...
    AAPiUartPrivate::set( ctx );

	return AAPI_SUCCESS;
}

void AAPiUartDevice::close()
{
    //----- CLOSE -----
    if (! is_open( ))
    {
...
int AAPiUartDevice::write(const uint8_t *buffer, uint32_t length)
{
    //----- TX BYTES -----
    if (! is_open( ))
    {
...
int AAPiUartDevice::read(uint8_t *buffer, uint32_t length)
{
...
void *AAPiUartPrivate::thread_func(void *arg)
{
    AAPiUartPrivate *priv = reinterpret_cast<AAPiUartPrivate *>( arg );
    assert( priv );
...
void AAPiUartPrivate::set(AAPiUartPrivate *ctx)
{
    std::lock_guard<AAPiUartPrivate::TMutex> lock( m_mtx );

	m_cache.push_back( ctx );
}

bool AAPiUartPrivate::remove(AAPiUartPrivate *ctx)
{
    std::lock_guard<AAPiUartPrivate::TMutex> lock( m_mtx );

    AAPiUartPrivate::TCache::const_iterator iter = m_cache.begin();
...
void AAPiUartPrivate::io_sig_handler (int status)
{
	(void)status;

    std::lock_guard<AAPiUartPrivate::TMutex> lock( m_mtx );

    AAPiUartPrivate::TCache::iterator iter = m_cache.begin();
    for (; iter != m_cache.end(); ++iter)
    {
		(*iter)->io_ready = true;
	}
...
int AAPiUartDevice::start(AAPiUartCallback *callback)
{
    if (! is_open( ))
    {
        return AAPI_E_INVALID_STATE;
...
bool AAPiUartDevice::is_open() const
{
    return ( m_priv != nullptr );
}

bool AAPiUartDevice::is_listening() const
{
